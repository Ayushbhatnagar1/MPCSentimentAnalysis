...
# Import necessary libraries if not already done so
import pandas as pd
from sklearn.feature_extraction.text import TfidfVectorizer
from nltk.corpus import stopwords

# Assuming your existing data is in a DataFrame named `data`
# with a column 'Cleaned_Text' that contains the preprocessed text for TF-IDF transformation
corpus = data['Cleaned_Text'].tolist()

# Create a TfidfVectorizer object with suitable parameters
tfidf_vectorizer = TfidfVectorizer(max_features=500,
                                   stop_words=stopwords.words('english'),
                                   ngram_range=(1, 2))

# Fit the vectorizer to the corpus and transform the text to TF-IDF features
tfidf_matrix = tfidf_vectorizer.fit_transform(corpus)

# Get the feature names (words or n-grams) from the vectorizer
...
# Ensure that both DataFrames are aligned by their indices
combined_data = pd.concat([data.reset_index(drop=True), tfidf_df.reset_index(drop=True)], axis=1)

# Now `combined_data` contains the original data along with the TF-IDF features
# You can inspect the first few rows to verify
print(combined_data.head())

# If you wish to save the combined data to a new Excel file, you can do so
output_file = 'combined_data_with_tfidf.xlsx'
combined_data.to_excel(output_file, index=False)

# Ensure that you run this in a Jupyter Notebook cell
